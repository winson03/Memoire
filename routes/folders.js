...
// Create a folder. Theme cycles. (Collections feature disabled.)
router.post('/', (req, res) => {
  ...
  const folder = Folders.create(req.user.id, name, theme, null);
  res.redirect('/folders?open=' + folder.id);
});

// Collections feature disabled — folder→collection move route unmounted.
