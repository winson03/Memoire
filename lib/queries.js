...
const Collections = {
  ...
  remove(id) {
    // Removing a collection un-files its folders; it does NOT delete folders/stories.
...
const Books = {
  ...
  all: db.prepare(BOOK_SELECT + ' ORDER BY b.id'),
  published: db.prepare(BOOK_SELECT + " WHERE b.status = 'published' ORDER BY b.id"),
  byAuthor: db.prepare(BOOK_SELECT + ' WHERE b.author = ? ORDER BY b.id'),
  byFolder: db.prepare(BOOK_SELECT + ' WHERE b.folder_id = ? ORDER BY b.id'),
  byUser: db.prepare(BOOK_SELECT + ' WHERE b.user_id = ? ORDER BY b.id'),

  findById(id) { return this.byId.get(id); },
  listAll() { return this.all.all(); },
  listPublished() { return this.published.all(); },
  listByAuthor(author) { return this.byAuthor.all(author); },
  listByFolder(folderId) { return this.byFolder.all(folderId); },
...
const Gallery = {
  ...
  findById(id) { return this.byId.get(id); },
  listForUser(userId, order = 'newest') {
    return (order === 'oldest' ? this.oldest : this.newest).all(userId);
  },

  create({ user_id, telegram_file_id, telegram_unique_id = null, telegram_message_id = null, mime = null, file_name = null, file_size = null }) {
    const info = db.prepare(`
      INSERT INTO gallery_images (user_id, telegram_file_id, telegram_unique_id, telegram_message_id, mime, file_name, file_size)
      VALUES (@user_id, @telegram_file_id, @telegram_unique_id, @telegram_message_id, @mime, @file_name, @file_size)
    `).run({ user_id, telegram_file_id, telegram_unique_id, telegram_message_id, mime, file_name, file_size });
    return this.findById(info.lastInsertRowid);
  },
