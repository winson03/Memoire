...
// Telegram rate-limits sends to a chat (~20/minute). Over the limit it replies
// 429 with `retry_after` seconds — a "slow down", not a real failure. We wait
// that long and retry so big batch uploads finish instead of dropping files.
...
// Send one method/field to one endpoint. `makeBody` returns a Buffer or a
// fresh readable stream — a factory so endpoint fallback can retry with a new
// stream (a consumed one can't be replayed). `size` sets knownLength so
// form-data can emit Content-Length without buffering the stream.
async function sendOnce(ep, method, field, makeBody, size, fileName, mime, retryLeft = RATE_LIMIT_RETRIES) {
  const form = new FormData();
  form.append('chat_id', CHAT_ID);
  const opts = { filename: fileName || 'upload', contentType: mime || 'application/octet-stream' };
  if (size != null) opts.knownLength = size;
  form.append(field, makeBody(), opts);
  ...
  try {
    ...
    // Rate-limited: retry after the delay Telegram asks for (fresh body each
    // time — a consumed stream can't be replayed). `retryLeft` guards the depth.
    if (status === 429 && retryLeft > 0) {
      const secs = (err.response.data && err.response.data.parameters && err.response.data.parameters.retry_after) || 1;
      await sleep((secs + 0.5) * 1000);
      return sendOnce(ep, method, field, makeBody, size, fileName, mime, retryLeft - 1);
    }
...
async function uploadBuffer(buffer, fileName, mime, opts = {}) {
  ...
  return sendWithFallbacks(primary, () => buffer, buffer.length, fileName, mime);
}

/**
 * Upload a file straight from disk, streaming it (no full-file buffer — a
 * 300 MB video would otherwise OOM a small host). Images still go through
 * uploadBuffer for the shrink step; everything else streams.
 */
async function uploadFile(srcPath, fileName, mime, opts = {}) {
  const m = (mime || '').toLowerCase();
  if (m.startsWith('image/')) {
    return uploadBuffer(await fs.promises.readFile(srcPath), fileName, mime, opts);
  }
  if (!isConfigured()) {
    throw new Error('Telegram is not configured (missing TELEGRAM_BOT_TOKEN / TELEGRAM_CHAT_ID).');
  }
  ...
  const primary = opts.asDocument
    ? { method: 'sendDocument', field: 'document', kind: methodForMime(mime, fileName).kind }
    : methodForMime(mime, fileName);
  return sendWithFallbacks(primary, () => fs.createReadStream(srcPath), size, fileName, mime);
}

// Try the type-appropriate method, then sendDocument, each across endpoints.
async function sendWithFallbacks(primary, makeBody, size, fileName, mime) {
  let result, field = primary.field, kind = primary.kind;
  try {
    result = await withFallback((ep) => sendOnce(ep, primary.method, primary.field, makeBody, size, fileName, mime));
  } catch (err) {
    ...
    result = await withFallback((ep) => sendOnce(ep, 'sendDocument', 'document', makeBody, size, fileName, mime));
    field = 'document';
  }
  ...
  return {
    ...
    file_size: file.file_size || size || null,
  };
}
...
async function streamTo(fileId, res, { mime, fileName, inline = true, range = null } = {}) {
  ...
  // If the ranged request fails (some file servers reject Range), fall back to
  // a plain full download so playback still works.
  const downloadUrl = `${ep.base}/file/bot${TOKEN}/${filePath}`;
  const get = (withRange) => axios.get(downloadUrl, {
    responseType: 'stream',
    timeout: 10 * 60 * 1000,
    headers: withRange && range ? { Range: range } : {},
