...
// a Telegram file_id, or a local filename — so no schema change is needed.
...
// saveFile: read the temp upload into a buffer and send it. The caller is
// responsible for cleaning up its own temp file (it does, in a finally block).
async function tgSaveFile(srcPath, fileName, mime) {
  const buffer = await fs.promises.readFile(srcPath);
  return telegram.uploadBuffer(buffer, fileName, mime);
}
...
console.log(`[storage] backend: ${USE_TELEGRAM ? 'telegram' : 'local disk'}`);
